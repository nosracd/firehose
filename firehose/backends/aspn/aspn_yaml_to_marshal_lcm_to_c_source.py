...
class AspnYamlToMarshalLCMToCSource(Backend):
    ...
    def generate(self) -> None:
        ...
        static AspnTypeIntegrity* marshal_{ASPN_PREFIX_LOWER}_lcm_type_integrity({ASPN_PREFIX_LOWER}_lcm_type_integrity* integrity, int length) {{
        ...
        static AspnTypeHeader marshal_{ASPN_PREFIX_LOWER}_lcm_type_header({ASPN_PREFIX_LOWER}_lcm_type_header header, {ASPN_PREFIX}MessageType type) {{
        ...
        static AspnTypeMetadataheader* marshal_{ASPN_PREFIX_LOWER}_lcm_type_metadataheader({ASPN_PREFIX_LOWER}_lcm_type_metadataheader lcm_msg, {ASPN_PREFIX}MessageType type) {{
    ...
    def process_data_pointer_field(
        ...
        if type_name == f"{ASPN_PREFIX}TypeIntegrity":
            self.current_struct.function_prep_buf.append(
                f"AspnTypeIntegrity* integrity = marshal_{ASPN_PREFIX_LOWER}_lcm_type_integrity(lcm_msg->integrity, lcm_msg->num_integrity);"
            )
    ...
    def process_simple_field(
        ...
        if field_type_name == f"{ASPN_PREFIX}TypeHeader":
            self.current_struct.function_prep_buf.append(
                f"AspnTypeHeader header = marshal_{ASPN_PREFIX_LOWER}_lcm_type_header(lcm_msg->header, {self.current_struct.struct_enum});"
            )
            self.current_struct.function_args.append("&header")
        elif field_type_name == f"{ASPN_PREFIX}TypeTimestamp":
            self.current_struct.function_prep_buf.append(
                f"AspnTypeTimestamp {field_name} = {{lcm_msg->{field_name}.elapsed_nsec}};"
            )
            self.current_struct.function_args.append(f"&{field_name}")
        elif field_type_name == f"{ASPN_PREFIX}TypeMetadataheader":
            self.current_struct.function_prep_buf.append(
                f"AspnTypeMetadataheader* {field_name} = marshal_{ASPN_PREFIX_LOWER}_lcm_type_metadataheader(lcm_msg->{field_name}, {self.current_struct.struct_enum});"
            )
            self.current_struct.function_args.append(f"{field_name}")
            self.current_struct.function_free_buf.append(
                f"aspn_type_metadataheader_free({field_name});"
            )
        elif field_type_name == f"{ASPN_PREFIX}TypeImageFeature":
            self.current_struct.function_prep_buf.append(
                f"""AspnTypeImageFeature* {field_name} = NULL;
                if (lcm_msg->has_observation_characteristics) {{
                    {field_name} = marshal_{ASPN_PREFIX_LOWER}_lcm_type_image_feature(&lcm_msg->{field_name});
                    }}"""
            )
            self.current_struct.function_args.append(f"{field_name}")
            self.current_struct.function_free_buf.append(
